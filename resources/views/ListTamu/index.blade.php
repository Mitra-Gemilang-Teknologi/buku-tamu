				...
				<table id="example2" class="table table-bordered table-hover">
					...
					<tbody>
						...
						<tr>
							...
							<td>
								@if (count($kunjungan->subServices) > 0)
								@foreach ($kunjungan->subServices as $subServices)
								<li>{{ optional($subServices->subLayanan)->sub_service_name ?? '-' }}</li>
								@endforeach
								@elseif (count($kunjungan->service) > 0)
								@foreach ($kunjungan->service as $service)
								<li>{{ optional($service->layanan)->service_name ?? '-' }}</li>
								@endforeach
								@endif
							</td>
